...
// ===== Constants =====
#define SPEEDHACK_TICKBASE_GRACE     60.0    // Seconds to suppress after tickbase clamp
...
// Tickbase manipulation tracking
static float player_last_tickbase_clamp[MAXPLAYERS + 1];
...
void lilac_speedhack_reset_client(int client)
{
    ...
    player_last_tickbase_clamp[client] = 0.0;
...
// ===== Hook point for tickbase manipulation module =====
// Call this from wherever tickbase clamping occurs (e.g. the function
// that logs "tickbase manipulation: N ticks ahead. Clamped.").
void lilac_speedhack_notify_tickbase_clamp(int client)
{
    player_last_tickbase_clamp[client] = GetGameTime();
}
...
public Action timer_check_speedhack(Handle timer)
{
    ...
    for (int client = 1; client <= MaxClients; client++) {
         ...
         * When the server clamps tickbase, the client was simulating
         * while disconnected. Cmd bursts after this are expected
         * recovery behavior, not speedhack. */
        if (player_last_tickbase_clamp[client] > 0.0
            && (now - player_last_tickbase_clamp[client]) < SPEEDHACK_TICKBASE_GRACE)
        {
            continue;
        }
